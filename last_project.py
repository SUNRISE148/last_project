...
class Player(GameSprite):
    ...
    def fire_left(self):
        bullet = Bullet(img_bullet_left, self.rect.centerx, self.rect.top,20,15, -15)
        bullets.add(bullet)
    def fire_right(self):
        bullet = Bullet(img_bullet_right, self.rect.centerx, self.rect.top,20,15, 15)
        bullets.add(bullet)
...
class Enemy(GameSprite):
    def update(self):
        ...
        if self.rect.x < 350 and self.rect.x > 300:
            ...
            if a == 0:
                self.rect.x = 700
                player_speed = randint(1,5)*-1
            if a == 1:
                self.rect.x = -50
                player_speed = randint(1,5)
            #self.rect.x = 500
            #self.player_speed = randint(1,5)*((a*2)-1)
            #self.rect.y = 400
            lost = lost+1
...
ship = Player(img_hero_left,300,400,80,100,12)
life_station = GameSprite('life.png', 300, 300,100,100,0)
stand = GameSprite('stand.png', 300, 500,100,50,0)
monsters = sprite.Group()
for i in range(1,6):
    a = randint(0,1)
    if a == 0:
        monster = Enemy(img_enemy, 700, 400,80,50,randint(1,5)*-1)
        monsters.add(monster)
    if a == 1:
        monster = Enemy(img_enemy,-50, 400,80,50,randint(1,5))
        monsters.add(monster)
...
while run:
    for e in event.get():
        if e.type == QUIT:
            ...
            if e.key == K_a:
                fire_direction = 1
                #ship.reset_direction("character_left.png",80,100)
                #ship.player_image = 'character_left.png'
                #ship = Player(img_hero_left,300,400,80,100,12)
            if e.key == K_d:
                fire_direction = 0
                #ship.reset_direction("character_right.png",80,100)
                #ship.player_image = 'character_right.png'
                #ship = Player(img_hero_right,300,400,80,100,12)
            if e.key == K_SPACE:
                if fire_direction == 1:
    ...
    if not finish:
        ...
        for c in collides:
            ...
            a = randint(0,1)
            if a == 0:
                monster = Enemy(img_enemy, 600, 400,80,50,randint(1,5)*-1)
                monsters.add(monster)
            if a == 1:
                monster = Enemy(img_enemy,-50, 400,80,50,randint(1,5))
                monsters.add(monster)
